...
Aller sur supabase.com → créer un compte gratuit
New project → nommer "qhse-mega" → choisir une région
Dans SQL Editor → coller et exécuter le SQL fourni dans l'application au premier démarrage
Dans Settings → API → noter l'URL et la clé `anon`
2. Déployer sur GitHub Pages (5 min)
Créer un dépôt GitHub (public)
Uploader `index.html`, `sw.js`, `manifest.json`, `README.md`
Settings → Pages → Source: main → Save
URL: `https://votre-username.github.io/qhse-mega/`
3. Configurer au premier démarrage
Ouvrir l'URL GitHub Pages
Un assistant de configuration s'affiche
Saisir l'URL Supabase et la clé API
Cliquer Connecter → créer votre compte → c'est parti!
Sécurité
